...
class _ContactsScreenState extends State<ContactsScreen> {
  ...
  List<Map<String, dynamic>> _smartLists = []; // from Supabase
  ...
  Future<void> _loadLeads() async {
    ...
    try {
      ...
      final data = await _db.from('leads').select(
        'id, lead_name, lead_email, lead_phone, lead_status, source, '        'notes, estimated_value, tags, date_added, last_message_at, '        'business_name, lead_address'
  ...
  void _applyFilter() {
    ...
    if (_activeList == 1) result = result.where((l) => l.status == 'New').toList();
      ...
      // Custom smart list — apply saved filters
  ...
  void _showAdd() => showModalBottomSheet(
    ...
    builder: (_) => _AddSheet(businessId: _businessId ?? 0, onSaved: () { Navigator.pop(context); _loadLeads(); }),
  );
    ...
    showDialog(
      ...
      builder: (_) => _ImportCsvDialog(
        ...
        onImported: (count) {
          ...
          _loadLeads();
          _snack('Imported $count contacts successfully!');
        },
      ),
    );
  }
  ...
  void _showManageSmartLists() {
    ...
    ).then((_) => _loadSmartLists());
  }

  void _snack(String msg) => ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text(msg), backgroundColor: AppTheme.brand, duration: const Duration(seconds: 2)));
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(color: AppTheme.brand.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
      ...
      child: MouseRegion(cursor: SystemMouseCursors.click,
        child: GestureDetector(onTap: onTap,
          child: Container(
            width: 36, height: 36,
            decoration: BoxDecoration(
              color: active ? AppTheme.brand.withOpacity(0.1) : AppTheme.pageBg,
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: active ? AppTheme.brand.withOpacity(0.4) : AppTheme.borderColor),
            ),
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(children: [
          ...
          // Divider
          Container(height: 20, width: 1, color: AppTheme.borderColor,
  ...
  Widget _filterDrop(String label, String value, List<String> items, ValueChanged<String?> onChange) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: value != 'All' ? AppTheme.brand.withOpacity(0.08) : AppTheme.pageBg,
        borderRadius: BorderRadius.circular(6),
        border: Border.all(color: value != 'All' ? AppTheme.brand.withOpacity(0.4) : AppTheme.borderColor),
      ),
  ...
  Widget _buildBulkBar() {
    return Container(
      ...
      child: Row(children: [
        ...
        _bulkBtn(Icons.local_offer_outlined, 'Tag', () => _snack('Bulk tag coming soon!')),
        const SizedBox(width: 8),
        _bulkBtn(Icons.email_outlined, 'Email', () => _snack('Bulk email coming soon!')),
        const SizedBox(width: 8),
        _bulkBtn(Icons.sms_outlined, 'SMS', () => _snack('Bulk SMS coming soon!')),
        const SizedBox(width: 8),
  ...
  Widget _buildRow(_Lead lead) {
    ...
    return GestureDetector(
      ...
      child: MouseRegion(
        ...
        child: Container(
          height: 56,
          color: selected ? AppTheme.brand.withOpacity(0.04) : AppTheme.cardBg,
          child: Row(children: [
            GestureDetector(
            ...
            if (_colCreated) Expanded(flex: 2, child: Padding(
              ...
              child: lead.createdAt != null
                  ? Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(_fmtDate(lead.createdAt!),
                        style: const TextStyle(color: AppTheme.textPrimary, fontSize: 12)),
                      Text(_fmtTime(lead.createdAt!),
  ...
  void _showColumnPicker() => showDialog(context: context, builder: (_) => AlertDialog(
    ...
    content: StatefulBuilder(builder: (ctx, setS) => Column(mainAxisSize: MainAxisSize.min, children: [
      _colTgl('Phone', _colPhone, setS, (v) { _colPhone = v; }),
      _colTgl('Email', _colEmail, setS, (v) { _colEmail = v; }),
      _colTgl('Created Date', _colCreated, setS, (v) { _colCreated = v; }),
      _colTgl('Last Activity', _colActivity, setS, (v) { _colActivity = v; }),
      _colTgl('Tags', _colTags, setS, (v) { _colTags = v; }),
      _colTgl('Source', _colSource, setS, (v) { _colSource = v; }),
      _colTgl('Est. Value', _colValue, setS, (v) { _colValue = v; }),
    ])),
  ...
  Widget _colTgl(String label, bool value, StateSetter setS, Function(bool) onChanged) =>
    SwitchListTile(
      ...
      onChanged: (v) { setS(() => onChanged(v)); setState(() => onChanged(v)); },
    );
    ...
    final step2 = await showDialog<bool>(
      ...
      builder: (ctx2) => AlertDialog(
        ...
        content: const Text(
          ...
          style: TextStyle(color: AppTheme.textSecondary, fontSize: 14, height: 1.5)),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx2).pop(false),
...
class _AddSheetState extends State<_AddSheet> {
  ...
  Future<void> _loadBusinessName() async {
    try {
      ...
      final biz = await db.from('businesses')
      ...
      if (biz != null && mounted) {
        setState(() {
          ...
          if (_bizCtrl.text.isEmpty && _businessName != null) {
            _bizCtrl.text = _businessName!;
          }
        });
      }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await _db.from('leads').insert({
        ...
        'estimated_value': _valueCtrl.text.trim().isEmpty
            ? null : double.tryParse(_valueCtrl.text.trim().replaceAll(r'$', '').replaceAll(',', '')),
      ...
      builder: (_, sc) => Container(
        ...
        child: Column(children: [
          ...
          Expanded(child: Form(key: _fk, child: ListView(controller: sc, padding: const EdgeInsets.all(24), children: [
            ...
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: _field(_valueCtrl, 'Estimated Value (\$)',
...
class _ImportCsvDialogState extends State<_ImportCsvDialog> {
  ...
  // Column mapping: our field -> CSV column index
  String? _mapName;
  String? _mapEmail;
  String? _mapPhone;
  String? _mapStatus;
  ...
  List<List<dynamic>> get _dataRows =>
      _hasHeader && _rows.length > 1 ? _rows.sublist(1) : _rows;
  ...
  Future<void> _import() async {
    if (_mapName == null) {
      setState(() => _error = 'Please map the Name column.');
      return;
    }
    ...
    try {
      ...
      for (final row in _dataRows) {
        ...
        if (rawVal != null) {
          estValue = double.tryParse(
            rawVal.replaceAll(r'$', '').replaceAll(',', '').trim());
        }

        batch.add({
          'lead_name': name,
          'lead_email': _val(row, _mapEmail),
          'lead_phone': _val(row, _mapPhone),
          'lead_status': _val(row, _mapStatus) ?? 'New',
          'source': _val(row, _mapSource) ?? 'Import',
          'business_name': _val(row, _mapBusiness),
          'lead_address': _val(row, _mapAddress),
          'notes': _val(row, _mapNotes),
      ...
      if (batch.isEmpty) {
        setState(() { _error = 'No valid rows found.'; _importing = false; });
        return;
      }
      ...
      setState(() { _error = e.toString(); _importing = false; });
    }
  }
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 10),
              const Expanded(child: Text('Import CSV',
          ...
          if (_error != null)
            Container(margin: const EdgeInsets.fromLTRB(20, 16, 20, 0),
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Checkbox(
                  value: _hasHeader, activeColor: AppTheme.brand,
                  onChanged: (v) => setState(() => _hasHeader = v!)),
                const Text('First row is a header',
                  style: TextStyle(color: AppTheme.textPrimary, fontSize: 13)),
                const Spacer(),
                Text('${_dataRows.length} rows to import',
                  style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
              ]),
              const SizedBox(height: 16),
              const Text('Map columns', style: TextStyle(
                color: AppTheme.textSecondary, fontSize: 11,
            ...
            child: Row(children: [
              ...
              Expanded(flex: 2, child: ElevatedButton(
                ...
                child: _importing
                    ? const SizedBox(width: 18, height: 18,
                        child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                    : Text('Import ${_dataRows.length} Contacts',
...
class _SmartListsDialogState extends State<_SmartListsDialog> {
  ...
  Future<void> _delete(int id) async {
    ...
    await _load();
    // onChanged is called when dialog closes, not on each delete
  }
  ...
  @override
  Widget build(BuildContext context) {
    // Check if current filters are worth saving
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 10),
              const Expanded(child: Text('Manage Smart Lists',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700,
                    color: AppTheme.textPrimary))),
              GestureDetector(onTap: () => Navigator.pop(context),
                child: const Icon(Icons.close, color: AppTheme.textSecondary, size: 20)),
          ...
          Padding(padding: const EdgeInsets.all(20), child: Column(
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Save current filters as a Smart List',
                  style: TextStyle(color: AppTheme.textPrimary, fontSize: 13,
                      fontWeight: FontWeight.w600)),
                const SizedBox(height: 4),
                Text(
                  hasActiveFilters
                    ? _describeFilters(widget.currentFilters)
                    : 'Set filters on the Contacts page first, then save them here.',
                  style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                if (hasActiveFilters) ...[
                  const SizedBox(height: 12),
                  Row(children: [
            ...
            const SizedBox(height: 20),
            const Text('Your Smart Lists',
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 11,
                  fontWeight: FontWeight.w700, letterSpacing: 0.8)),
            const SizedBox(height: 10),

            // List of saved smart lists
            if (_loading)
              ...
              Container(
                ...
                child: const Center(child: Text(
                  'No smart lists yet. Set filters on the Contacts page and save them here.',
                  style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
              ...
              ...(_lists.map((list) => Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(
                    ...
                    Text(_describeFilters(
                        Map<String, dynamic>.from(list['filters'] as Map)),
